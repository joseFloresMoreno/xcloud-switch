...
int xbox_auth_get_streaming_tokens(XboxAuthContext* ctx) {
    if (!ctx || !ctx->xbl_token[0]) return -1;
    ...
    /* --- GSSV XSTS (relying party: http://gssv.xboxlive.com) --- */
    {
        ...
        if ((code != 200 && code != 201) || !resp.buf) {
            ...
            if (resp.buf) json_long(resp.buf, "XErr", &xerr);
            snprintf(ctx->error, sizeof(ctx->error),
                     "GSSV XSTS fallido (HTTP %d, XErr %ld)", code, xerr);
            dynbuf_free(&resp);
            return -1;
        }
    ...
    /* --- xhome streaming login --- */
    {
        ...
        char auth_hdr[4096 + 32];
        snprintf(auth_hdr, sizeof(auth_hdr), "Authorization: Bearer %s", ctx->gssv_token);
        ...
        if ((code != 200 && code != 201) || !resp.buf) {
            snprintf(ctx->error, sizeof(ctx->error),
                     "xhome login fallido (HTTP %d)", code);
            dynbuf_free(&resp);
            return -1;
        }

        printf("[AUTH] xhome login HTTP %d — %.120s\n", code, resp.buf);

        if (json_str(resp.buf, "gsToken", ctx->gs_token, sizeof(ctx->gs_token)) != 0) {
            snprintf(ctx->error, sizeof(ctx->error), "No se encontro gsToken en respuesta xhome");
            dynbuf_free(&resp);
            return -1;
        }
