...
int xcloud_api_send_sdp(XCloudApiClient* client, const char* session_id,
    ...
    char path[256];
    snprintf(path, sizeof(path), "/v5/sessions/%s/%s/sdp/ice",
             type_str(client->type), session_id);
    int body_size = (int)strlen(sdp) + 256;
    char* body = (char*)malloc(body_size);
    if (!body) return -1;
    snprintf(body, body_size,
             "{\"offerContext\":{\"offer\":\"%s\","
             "\"iceUfrag\":\"\",\"icePwd\":\"\"},"
             "\"candidates\":[]}",
             sdp);
...
int xcloud_api_send_ice(XCloudApiClient* client, const char* session_id,
    ...
    if (!client || !session_id || !ice) return -1;
    char path[256];
    snprintf(path, sizeof(path), "/v5/sessions/%s/%s/ice",
             type_str(client->type), session_id);
    int body_size = (int)strlen(ice) + 32;
    char* body = (char*)malloc(body_size);
    if (!body) return -1;
    snprintf(body, body_size, "{\"candidates\":[%s]}", ice);
    int ret = do_request(client, "POST", path, body, response, response_size);
    free(body);
    return ret;
}
